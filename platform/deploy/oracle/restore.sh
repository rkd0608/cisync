#!/usr/bin/env bash
# Sauron Phase-1 restore drill — RUNBOOK-oracle §7 (WEEKLY, rehearsed).
...
#
# Usage:  AGE_PRIVATE_KEY_FILE=./age.key ./restore.sh backups/sauron-pg-*.dump.gz.age
set -euo pipefail
...
DRILL_CONTAINER="sauron-restore-drill"
...
scratch=$(mktemp -d /tmp/sauron-restore.XXXXXX)
...
docker run -d --rm --name "$DRILL_CONTAINER" \
	-e POSTGRES_USER=sauron -e POSTGRES_PASSWORD=drill-only \
	-e POSTGRES_DB=sauron "$RESTORE_IMAGE" >/dev/null
for i in $(seq 1 30); do
	docker exec "$DRILL_CONTAINER" pg_isready -U sauron >/dev/null 2>&1 && break
...
docker exec -i "$DRILL_CONTAINER" pg_restore -U sauron -d sauron --no-owner --exit-on-error \
...
for tbl in ctrl.ledger ctrl.ledger_checkpoints ingest.deliveries ctrl.decisions fleet.execution_jobs ghconn.check_reports; do
	n=$(docker exec "$DRILL_CONTAINER" psql -U sauron -d sauron -tAc \
...
if [ -n "$live" ]; then
	live_n=$(docker exec "$live" psql -U sauron -d sauron -tAc "SELECT count(*) FROM ctrl.ledger" 2>/dev/null || echo "?")
...
[drill] SCRATCH RESTORE PASSED. Manual swap into place (REHEARSE BEFORE YOU NEED IT):
  ...
  2. docker volume inspect sauron_pgdata   # note mountpoint
  ...
  5. docker volume create sauron_pgdata && start ONLY postgres:
     docker compose -f docker-compose.prod.yml --env-file .env.prod up -d postgres
  6. docker exec -i $(docker compose ... ps -q postgres) \
       pg_restore -U sauron -d sauron --no-owner --clean --if-exists < /tmp/dump.fc
