// Package redact is the log-output secret scrubber required by
// THREAT_MODEL B3: no secret ever reaches a log sink or an event payload.
// Module-local copy of the ingest scrubber's value patterns (Go internal
// visibility rules forbid cross-service imports); pattern classes stay in
// lockstep by review.
package redact
...
// String scrubs every secret-looking substring from s. Applied to runner
// log excerpts BEFORE classification inputs are embedded in ledger events
// (T1/B3) and to every log line via Writer.
