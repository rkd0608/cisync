...
// tracking versions in ingest.schema_migrations. Boot-time migration keeps
// the compose stack self-provisioning (schema ingest is exclusive-write).
func (s *PGStore) Migrate(ctx context.Context, dir string) error {
	if _, err := s.pool.Exec(ctx, `CREATE SCHEMA IF NOT EXISTS ingest`); err != nil {
		return fmt.Errorf("pg store: ensure schema: %w", err)
	}
	if _, err := s.pool.Exec(ctx,
		`CREATE TABLE IF NOT EXISTS ingest.schema_migrations (version integer PRIMARY KEY, applied_at timestamptz NOT NULL DEFAULT now())`,
	); err != nil {
	...
	for _, m := range migrations {
		...
		if err := s.pool.QueryRow(ctx,
			`SELECT EXISTS(SELECT 1 FROM ingest.schema_migrations WHERE version=$1)`, m.version,
		...
		if err := pgx.BeginFunc(ctx, s.pool, func(tx pgx.Tx) error {
			...
			_, err := tx.Exec(ctx, `INSERT INTO ingest.schema_migrations (version) VALUES ($1)`, m.version)
			return err
		}); err != nil {
			return err
		}
...
func MigrationsDir() string {
	for _, dir := range []string{"migrations", "services/ingest/migrations"} {
